...
export const DriveContinusReport: React.FC = () => {
  ...
  const seachParts = [
    ...
    {
      title: _const.string?.searchReportTitle?.driverContinusMinute,
      key: "minutes",
      required: true,
      addonAfter: "Tiếng",
    },
  ...
  const getData = useCallback((searchVals: IReportSearch) => {
    if (
      ...
      const driverContinusPromise = getDriveContinusRpService?.({
        ...
        minutes: (searchVals?.minutes || 0) * 60 || 0.001,
        ...
        .then(([dFb]: any) => {
          if (dFb?.result) {
            ...
            const dataS: IColumns[] = data?.map?.((drive, index) => {
              ...
              const startCoor = drive?.start_location?.split?.(",")
              const endCoor = drive?.end_location?.split?.(",")
              ...
              return {
                ...
                driverName:
                  driverOject?.[drive?.license_number]?.name ||
                ...
                start_address: drive?.start_address || (
                  <AddressCallback
                    ...
                    lat={Number(startCoor?.[0])}
                    lng={Number(startCoor?.[1])}
                  />
                ),
                end_address: drive?.end_address || (
                  <AddressCallback
                    address={drive?.start_address}
                    lat={Number(endCoor?.[0])}
                    lng={Number(endCoor?.[1])}
                  />
                ),
    ...
    <div className="h-full  flex flex-col gap-2">
      ...
      <div className="flex-1 main-rp-table shadow pb-2">
        <TableC
          ...
          exportExcel={{
            fileName: "BAO_CAO_THOI_GIAN_LAI_XE_LIEN_TUC",

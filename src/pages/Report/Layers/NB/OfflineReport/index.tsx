...
import { IOfflineRecord } from "../../../../../_types/deviceType"

export const OfflineReport: React.FC = () => {
  ...
  const getData = useCallback((searchVals: IReportSearch) => {
    if (
      ...
      setIsLoading(true)
      const reportPromise = offlineReportService?.({
      ...
      Promise.all([reportPromise])
        .then(([reportFb]: any) => {
          if (reportFb?.result) {
            const data: IOfflineRecord[] = reportFb?.data

...
import { getSpeedReportService } from "../../../../../services/reportServices"
...
import { IRouteData, ISpeedRecord } from "../../../../../_types/deviceType"

export const TT73Speed: React.FC = () => {
  ...
  const getData = useCallback((searchVals: IReportSearch) => {
    if (
      ...
      const reportPromise = getSpeedReportService?.(
        searchVals?.devices?.[0],
        searchVals?.startTime,
        searchVals?.endTime,
        store?.getState?.()?.user?.access?.userInfo?.id,
      )

      Promise.all([reportPromise])
        .then(([reportFb]: any) => {
          if (reportFb?.result) {
            const data: ISpeedRecord[] = reportFb?.data?.speed
            const vehicleName: string = reportFb?.data?.vehicle_name

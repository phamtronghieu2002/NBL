...
export const TT73SynByDriver: React.FC = () => {
  ...
  const seachParts = [
    ...
    // },
    {
      title: _const.string?.searchReportTitle?.driver,
      key: "multiDriver",
      required: true,
    },
  ]
  ...
  const getData = useCallback((searchVals: IReportSearch) => {
    if (
      ...
      searchVals?.drivers?.length
    ) {
      const body = {
        endTime: searchVals?.endTime,
        startTime: searchVals?.startTime,
        licenseNumber: searchVals?.drivers?.join?.(","),
        userId: store?.getState()?.user?.access?.userInfo?.id,
      }

      setIsLoading(true)
      const reportPromise = TT73SynByDriverReportService?.(body)

      Promise.all([reportPromise])
        .then(([reportFb]: any) => {
          if (reportFb?.result) {
            const data: ISynByDriverReport[] = Object?.values?.(reportFb?.data)
            ...
            const dataS: IColumns[] = data?.map?.((record, index) => {
              ...
              return {
                ...
                drivingPlate: record?.license_number,
                note: "-",
                driverName:
                  driverOject?.[record?.license_number]?.name ||
                ...
                over4HourDrive: record?.continuous_4 || 0,
              }
            })

...
import {
  getLevelAddModuleService,
  getLevelAddPermissionService,
  getLevelModuleService,
  getLevelPermissionService,
  getLevelRemoveModuleService,
  getLevelRemovePermissionService,
  updateSortLevelService,
} from "../../../../../services/dev_levelServices"
import { api } from "../../../../../_helper"
import { _const } from "../../../../../_constant"
import { RegisterPermissionModal } from "../../../../../conponents/modals/RegisterPermissionModal"
import { RegisterModuleModal } from "../../../../../conponents/modals/RegisterModuleModal"
      ...
      return (
        <EditInline
          ...
          onSave={(value, setState, setIsloading) => {
            setIsloading?.(true)
            updateSortLevelService?.(record?.data?.id, { sort: value })
              ?.then?.((fb: any) => {
                if (fb?.result) {
                  api?.message?.success(
        ...
        <div className="flex items-center">
          ...
          <RegisterPermissionModal
            ...
            getDefaultValPromise={() =>
              getLevelPermissionService(record?.data?.id)
            }
          ...
          <RegisterModuleModal
            ...
            getDefaultValPromise={() => getLevelModuleService(record?.data?.id)}
            title={`Gán MENU cho vai trò ${record?.data?.name}`}
            onFinish={(values = [], preVal = [], action, setIsLoading) => {
              if (values) {
                const addList = values?.filter?.(
                  (val) => !preVal?.includes?.(val),
                ...
                let addPromise = addList?.length
                  ? getLevelAddModuleService({
                      id: record?.data?.id,
                      modules: JSON.stringify(addList || []),
                    })
                  : Promise.resolve(true)
                ...
                let removePromise = removeList?.length
                  ? getLevelRemoveModuleService({
                      id: record?.data?.id,
                      modules: JSON.stringify(removeList || []),
                    })
                  : Promise.resolve(true)
